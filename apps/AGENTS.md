...
Never hand-edit `types.gen.ts` or `sdk.gen.ts`; `bun run gen-api` from the repo root regenerates both from the server's endpoints, with the dev server up. When they're stale, write code against the types as they'll be after a regen rather than authoring stand-ins.
