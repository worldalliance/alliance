...
- **Source form deleted:** the server refuses to delete a form while another form's current version reads it, and names those forms. The user chose this guard when asked during review, over the spec's normal-failure-path preference. A source deleted anyway, one only older versions read, makes the history endpoint return 404. The renderer says the form it reads was deleted and offers no retry, since a retry would fetch the same 404; old responses reviewed or reopened after the delete reach this. Validation reports the form as missing. The guard and a save that starts reading the form take no lock, so two admins deleting and saving in the same instant can still leave a form reading a deleted one. It then behaves like any other deleted source; locking the save path wasn't worth it for a race that needs two admins at once.
