...
The home feed keeps an activity only when `buildOutputFormResponse` returns an output, and hands that output on to the DTO, so it builds each output once and reports a malformed list answer once. `hasPublicOutputAnswer` alone lets through activities whose view shows nothing, since it ignores the view. Like the renderer, it counts an answer only when its `publicAnswers` entry is `true` and it isn't empty (`isOutputValueMissing`), so an activity whose answers are all unflagged or empty can't show an output of display blocks alone. Building the output costs more for candidates the feed turns down, but `hasPublicOutputAnswer` still turns away most of them before any formula runs.
