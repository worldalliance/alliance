...
The home feed keeps an activity only when `buildOutputFormResponse` returns an output, and hands that output on to the DTO, so it builds each output once and reports a malformed list answer once. `hasPublicOutputAnswer` alone lets through activities whose view shows nothing, since it ignores the view. It counts an answer by the renderer's own rule, `isOutputAnswerShown` in `common/src/forms/output-resolution.ts` (a `true` flag and a value, and for a list a row the view draws), over the same answers the renderer reads (`savedOutputAnswers`, which drops the answers of fields the form hides). An activity whose answers are all unflagged, empty or hidden can't show an output of display blocks alone. A public list answer that isn't a list of rows gets `buildOutputFormResponse` as far as the malformed list report, and the activity is then left out unless an answer counts. It parses the stored response first for the visibility verdicts those conditions read. The feed doesn't also require the built output to draw an answer: a view of display blocks alone ("You scored #{score}") shows once the respondent shares an output answer, even one the view doesn't draw. Building the output costs more for candidates the feed turns down, but `hasPublicOutputAnswer` still turns away most of them before any formula runs.
