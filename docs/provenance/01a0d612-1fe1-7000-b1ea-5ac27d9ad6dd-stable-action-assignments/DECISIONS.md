...
Some cohort paths fold contract state into membership: `computeIsTaggedOrInManualCohort` requires `hasActiveContractInFullRange` on the tag path. The resolver evaluates membership without that contract term, so contract changes keep flowing through `computeAssignmentCore` at read time and a contract-less member gets no row instead of a permanent exclusion.
...
Record the deploy time as the cutover. The catch-up job covers only actions launched after it. Earlier actions belong to the stage 3 backfill; otherwise catch-up would apply the processing-failure exception to historical actions.
