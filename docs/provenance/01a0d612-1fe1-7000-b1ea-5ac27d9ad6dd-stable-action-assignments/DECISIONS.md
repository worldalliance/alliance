...
Each resolved-after-deadline batch logs a warning; it only occurs when earlier processing failed, so it is the catch-up backlog alert. The pass resolves and writes each action separately, so an action whose cohort fails to resolve logs an error without holding back the others.
