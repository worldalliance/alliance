...
10. **Signup capture.** Web signup, mobile signup, and the web OAuth start
    pass the detected zone through `signupTimeZone` in
    ...
    zone and a refused one would fail the whole signup. Mobile OAuth sends no
    zone: it only signs in, since its start carries no referral code and OAuth
    creates an account only with one, so the server's `DEFAULT_TIME_ZONE` in
    its state never reaches a new account. The server's `DEFAULT_TIME_ZONE`
    fallback for a web OAuth start with no zone stays: the web has always sent
    one. What remains is moving mobile signup's detection onto
    `getDeviceTimeZone()`.
