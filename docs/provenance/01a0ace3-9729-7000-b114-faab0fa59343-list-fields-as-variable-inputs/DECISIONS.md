...
`stripHiddenAnswers` drops a cell whose sub-field is hidden for its row, checked against the form's answers with the row's cells on top, the same merge the list renderers and `getListSubFieldErrors` use. The resolver reads cells as it gets them and has no visibility check of its own.

- The live form (web and mobile) resolves variables from `effectiveFormData`, so a hidden cell reads as `undefined` even while raw `formData` still holds its value.
- In read-only mode, `stripHiddenAnswers` keeps answered cells, as it keeps answered fields, so a review's variables agree with the rows it shows.
...
- An output view block's own condition reads only the answers of fields `isVisibleInSavedResponse` shows, so a block can't show text that depends on an answer the view hides. It judges each field the way `isAnswerShown` does rather than handing the block's extras a field lookup: `isElementCurrentlyVisible` would evaluate an account-state condition as false and hide a field the respondent saw. Public or not doesn't matter here, as a block's condition already read private answers.
- A variable the view draws reads the same answers, so `#{name}` can't show a value computed from an answer the view hides.
...
The existing renderability check already rejects a formula that ends on an array or a record, and the tests cover `input1`, `input1[0]`, and `input1.map(p => p.roles)`. Its hint names `label` when the value has a readable one, as a choice and a city do, and otherwise the first property that reads as text, a number or yes/no, so a row gets `item.name` even when a multi-select comes first. It suggests `join` only for a list of text, numbers or yes/no, since joining rows or nested lists would show `[object Object]`, and otherwise suggests `.length`. The hint says what to add to the end of the formula rather than naming an input, because the value that failed is usually one the formula built, like `input1.map(p => p.name)`, and an author who pasted `input1.join(', ')` from the old wording would get the join error on rows. When the formula's outermost part is an operator, like `input1 || []`, the hint says to wrap it in parentheses first, because text added after `a || b` attaches only to `b`. It names only keys that can follow a dot, so a record built with a quoted key like `'first name'` gets another key or none. When the value can be one of several records or lists, like `flag ? input1[0] : input2`, the hint names only a key every one of them has. When it can be a list or a record, like `flag ? input1 : input1[0]`, the hint gives each part its own fix, `.length` for the list and a key for the record, because no one suffix works on both. When it can also be text, a number or yes/no, the hint says to add the fix to the part that gives the list or record, because a suffix on the whole formula would fail on the readable part. Runtime formatting didn't change. A formula that passes validation can't end on a list or record, and changing `formatVariableValue` would change existing scalar behavior that a test pins down.

`join` carries a `this` type in `formula-lib.ts`, so joining a list of rows, choices or lists is a type error rather than a formula that passes and shows `[object Object]`. TypeScript's wording for that error names the `this` context, so `variable-formula-check.ts` replaces it with one that says to name a part of each item first. Its example names a key the items have, the one the renderability hint picks: `item.name` for a row, `item.label` for a choice. A fixed `item.label` would send an author joining rows to a missing-property error. A list of lists gets no example. `join` is the only library member whose `this` type can fail to match, and a test in `formula-lib.test.ts` holds that, so the replacement can't catch anything else.
...
An input kind the running build doesn't know fails the variable rather than throwing, so an older app build that meets a kind added later shows the "This form can't be displayed" notice instead of crashing. An admin tab left open across a deploy can meet one too. Saving there reports the input as an error that says to reload, next to the form's other errors. A field kind it doesn't know can't be picked as an input, a variable that reads one fails the same way, and saving reports that input as an error that says to reload. A list input that names a sub-field of a kind it doesn't know fails and gets the same reload error. Reading it as `undefined` would pass for an unanswered field, so a formula's `??` fallback would show on an answered form.
