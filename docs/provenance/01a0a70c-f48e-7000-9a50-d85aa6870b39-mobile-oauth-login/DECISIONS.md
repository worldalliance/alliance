...
- `/mobile/oauth-callback` is a real page on the web app. Android hands a verified link straight to the app, so the page renders only when that didn't happen. The handoff can't reach the app from there, and `expo-web-browser` on Android ends the session as dismissed once the app is back in front, so the page says sign-in didn't finish and points to trying again or email and password. It doesn't tell the member to open the app.
